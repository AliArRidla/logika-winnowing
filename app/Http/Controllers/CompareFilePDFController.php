...
class CompareFilePDFController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $pdf = $pdfParser->parseFile(base_path() . '/public/uploads/' . $request->file('pdf')->getClientOriginalName());
        $pdf2 = $pdfParser->parseFile(base_path() . '/public/uploads/' . $request->file('pdf2')->getClientOriginalName());
         
        $text = $pdf->getText();
        $text2 = $pdf2->getText();
        // dd($text);
        
        $n = $request->input('ngram');
        ...
        return view('compare-pdf.hasil')->withresult($result);
    }
}
